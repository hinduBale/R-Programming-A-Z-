...
my.df <- data.frame(Countries_2012_Dataset, Codes_2012_Dataset, Regions_2012_Dataset)

head(my.df)
tail(my.df)
colnames(my.df) <- (c("Country", "Code", "Region"))
...
merged.df <- merge(myFile, my.df, by.x = "Country.Code", by.y = "Code")
head(merged.df, 15)
